...
## 更新
* 2.2版本：
  * 添加功能：全站需要密码，必须输入密码后才能访问反向代理网站
  * 修正了静态文件缓存中的问题（其中将.htaccess文件还原为了初始的版本）
  * 修正了程序放在二级目录下时的问题

* 2.1版本：
  * 将maxredirs次数设置为0，让跳转由客户浏览器而非服务器完成，避免某些网站在跳转过程中设置的cookies丢失的问题
  * 在header中替换域名，避免网站返回301的时候被重定向到原始网站
